...
# Preset definitions
#
# Each preset is one row Aptiro can create on demand for a user who
# clicks "Seed presets". They are NEVER created automatically — that
# would surprise users who already curated their strategy. Seeding is
# idempotent by (owner_id, name).
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Pydantic schemas (additive — none of the existing schemas are
# changed except StrategyUpsert/StrategyRead gaining `score_threshold`,
# which is documented separately in phase4_app_py_diff.md).
# ---------------------------------------------------------------------
...
@strategies_router.post("", response_model=StrategyRead,
                        status_code=201)
...
def delete_strategy(sid: str,
    ...
    session.commit()
    if was_active:
        # Promote the most-recently-updated remaining strategy to
        # active rather than leaving the user with no active row
        # (the next call to _active_strategy would otherwise spawn a
        # fresh default and surprise them).
        remaining = session.exec(select(Strategy).where(
            Strategy.owner_id == _uid())).all()
        if remaining:
            remaining.sort(key=lambda r: r.updated_at or _now(),
                           reverse=True)
...
@strategies_router.get("/{sid}/preview",
                       response_model=StrategyPreviewOut)
...
@strategies_router.post("/seed-presets", response_model=SeedPresetsOut,
                        status_code=201)
...
# ===== Phase 4 strategies (appended by deploy) =====
# All routes registered via app.include_router(strategies_router) at the
# bottom of this block. strategies_router is defined here, so this call
# is safe — it executes AFTER the definition.
